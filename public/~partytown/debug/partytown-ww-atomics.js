...
((self) => {
  ...
  const getConstructorName = (obj) => {
    ...
    try {
      const zoneJsConstructorName =
        null ===
          (_c =
            null === (_b = null == obj ? void 0 : obj.__zone_symbol__originalInstance) || void 0 === _b
              ? void 0
      ...
      ? 'string' === type || 'boolean' === type || 'number' === type || null == value
        ...
        : 'function' === type
          ...
          : (added = added || new Set()) && Array.isArray(value)
            ? added.has(value)
              ? [1, []]
              : added.add(value) && [1, value.map((v) => serializeForMain($winId$, $instanceId$, v, added))]
            : 'object' === type
              ? value[InstanceIdKey]
    ...
    if (serializedValueTransfer) {
      ...
      if (5 === serializedType) {
        ...
        return new Proxy(new Event(obj.type, obj), {
          get: (target, propName) =>
            propName in obj ? obj[propName] : 'function' == typeof target[String(propName)] ? noop : target[String(propName)],
        });
      }
      if (2 === serializedType) {
        return obj;
  ...
  const getter = (instance, applyPath, groupedGetters, rtnValue) => {
    ...
    ((target, applyPath, rtnValue, restrictedToWorker = false, groupedGetters = false) => {
      if (webWorkerCtx.$config$.logGetters) {
        try {
          const msg = `Get ${getTargetProp(target, applyPath)}, returned: ${getLogValue(applyPath, rtnValue)}${
  ...
  const createNodeCstr = (win, env, WorkerBase) => {
            ...
            if (scriptContent) {
              if (isScriptJsType(scriptType)) {
                ...
                const loadOnMainThread =
                  scriptId &&
                  (null === (_b = null === (_a = config.loadScriptsOnMainThread) || void 0 === _a ? void 0 : _a.includes) ||
                  void 0 === _b
                    ? void 0
                    : _b.call(_a, scriptId));
                if (loadOnMainThread) {
    ...
    cachedTreeProps(
      WorkerNode,
      commaSplit('childNodes,firstChild,isConnected,lastChild,nextSibling,parentElement,parentNode,previousSibling'),
    );
    win.Node = WorkerNode;
  };
        ...
        constructor() {
          ...
          ((win, nodeCstrs) => {
            ...
            win.customElements = {
              ...
              whenDefined: (tagName) =>
                registry.has(tagName) ? Promise.resolve() : callMethod(win, ['customElements', 'whenDefined'], [tagName]),
          ...
          commaSplit('atob,btoa,crypto,indexedDB,setTimeout,setInterval,clearTimeout,clearInterval').map((globalName) => {
            ...
            if (!(globalName in win)) {
              ...
              null != value &&
                (win[globalName] = 'function' != typeof value || value.toString().startsWith('class') ? value : value.bind(self));
            }
          });
            ...
            $window$: new Proxy(win, {
              get: (win, propName) => {
                ...
                if ('string' != typeof propName || isNaN(propName)) {
                  return (
                    null === (_a = webWorkerCtx.$config$.mainWindowAccessors) || void 0 === _a ? void 0 : _a.includes(propName)
                  )
  ...
  const createEnvironment = (
    { $winId$: $winId$, $parentWinId$: $parentWinId$, $url$: $url$, $visibilityState$: $visibilityState$ },
    isIframeWindow,
            ...
            try {
              ...
              if (rsp.ok) {
                ...
                let shouldExecute = javascriptContentTypes.some((ct) => {
                  ...
                  return null ===
                    (_c =
                      null === (_a = null == responseContentType ? void 0 : responseContentType.toLowerCase) || void 0 === _a
                        ? void 0
