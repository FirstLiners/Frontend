...
((window) => {
                          ...
                          : startsWith(cstrName, 'CSS') && cstrName.endsWith('Rule')
                            ...
                            : 'CSSStyleDeclaration' === cstrName
                              ...
                              : 'Attr' === cstrName
                                ...
                                : value.nodeType
                                  ? [3, [$winId$, getAndSetInstanceId(value), getNodeName(value), prevInstanceId]]
    ...
    for (; i < totalTasks; i++) {
      try {
        ...
        if (1 === applyPath[0] && applyPath[1] in winCtxs[winId].$window$) {
          setInstanceId(
            new winCtxs[winId].$window$[applyPath[1]](...deserializeFromWorker(worker, applyPath[2])),
            task.$instanceId$,
          );
        } else {
    ...
    try {
      if (isValidMemberName(memberName) && isNaN(memberName[0]) && 'all' !== memberName) {
        ...
        if ('function' === memberType) {
          (String(value).includes('[native') || Object.getPrototypeOf(implementation)[memberName]) &&
  ...
  const addGlobalConstructorUsingPrototype = ($interfaces$, mainWindow, cstrName) => {
    void 0 !== mainWindow[cstrName] &&
      $interfaces$.push([cstrName, 'Object', Object.keys(mainWindow[cstrName].prototype).map((propName) => [propName, 6]), 12]);
  };
  let worker;
  ((receiveMessage) => {
    ...
    return swContainer.getRegistration().then((swRegistration) => {
      swContainer.addEventListener('message', (ev) =>
        receiveMessage(ev.data, (accessRsp) => swRegistration.active && swRegistration.active.postMessage(accessRsp)),
      );
      return (worker, msg) => {
        0 === msg[0]
